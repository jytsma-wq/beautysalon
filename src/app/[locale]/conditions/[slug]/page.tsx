...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildSeoMetadata({
    ...
    description: `${condition.shortDescription} Silk Beauty Salon, Batumi.`,
    keywords: [condition.name, `${condition.name} treatment Batumi`, `${condition.name} Georgia`, ...localSeoKeywords],
    image: condition.image,
    imageAlt: `${condition.name} | Silk Beauty Salon`,
  });
}
...
export default async function ConditionPage({ params }: Props) {
  ...
  const tCommon = await getTranslations({locale: resolvedParams.locale, namespace: 'common'});
  const tNav = await getTranslations({locale: resolvedParams.locale, namespace: 'nav'});
  const condition = await getConditionBySlug(resolvedParams.slug, resolvedParams.locale);
      ...
      <section className="bg-[#f7f2eb] pt-[170px] md:pt-[188px]">
        <div className="container-custom py-16 md:py-20">
          ...
          <nav className="mb-8 flex items-center gap-2 text-[0.68rem] uppercase tracking-[0.18em] text-stone-500">
            <Link href="/" locale={resolvedParams.locale} className="hover:text-[#241f1b]">
              {tCommon('home')}
            </Link>
            <ChevronRight className="h-3.5 w-3.5" />
            <Link href="/conditions" locale={resolvedParams.locale} className="hover:text-[#241f1b]">
              {tCommon('conditions')}
            </Link>
            <ChevronRight className="h-3.5 w-3.5" />
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              >
                <Link href="/book" locale={resolvedParams.locale}>
                  {t('bookConsultation')}
                </Link>
              </Button>
            </div>
            ...
            <div className="relative aspect-3/4 w-full overflow-hidden rounded-[8px]">
              <Image
                src={condition.image}
                alt={`${condition.name} | Silk Beauty Salon`}
                fill
            ...
            <div className="lg:col-span-2">
              ...
              <div className="mb-12">
                <h2 className="mb-6 font-sans text-2xl font-light text-[#241f1b] md:text-3xl">
                  {t('aboutCondition', {name: condition.name})}
                </h2>
