...
import { chairRentalContent } from '@/data/chair-rental';
import { siteConfig } from '@/data/site-config';
import { buildSeoMetadata } from '@/lib/seo';

const useCaseIcons = [BriefcaseBusiness, CalendarCheck, Repeat2];

function getContent(locale: string) {
  const validLocale = locales.includes(locale as Locale) ? (locale as Locale) : 'en';
  return chairRentalContent[validLocale];
}
...
export default async function ChairRentalPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <>
      <section className="bg-[#f7f2eb]" aria-labelledby="chair-rental-title">
      ...
      <section className="section-spacing" aria-labelledby="chair-use-cases-title">
        <div className="container-custom">
          <div className="mb-12 max-w-3xl">
            <h2 id="chair-use-cases-title" className="mb-5 font-sans text-3xl font-light text-[#241f1b] md:text-4xl">
              {content.useCasesHeading}
            </h2>
            <p className="text-base leading-7 text-stone-700">{content.useCasesIntro}</p>
          </div>
          ...
          <div className="grid gap-8 md:grid-cols-3">
            {content.useCases.map((useCase, index) => {
              ...
              return (
                <article key={useCase.title} className="border-t border-[#cdbca9] pt-6">
                  <Icon className="mb-8 h-7 w-7 text-[#8d6f58]" strokeWidth={1.4} aria-hidden="true" />
                  <h3 className="mb-3 font-sans text-xl font-light text-[#241f1b]">{useCase.title}</h3>
                  <p className="text-sm leading-6 text-stone-600">{useCase.description}</p>
                </article>
              );
            })}
          </div>
      ...
      <section className="section-spacing bg-[#f7f4f0]" aria-labelledby="chair-confirmation-title">
        <div className="container-custom grid gap-12 lg:grid-cols-[42%_58%] lg:gap-16">
          <div>
            <Armchair className="mb-8 h-8 w-8 text-[#8d6f58]" strokeWidth={1.4} aria-hidden="true" />
            <h2 id="chair-confirmation-title" className="mb-5 font-sans text-3xl font-light text-[#241f1b] md:text-4xl">
              {content.confirmationHeading}
            </h2>
            <p className="text-base leading-7 text-stone-700">{content.confirmationIntro}</p>
          </div>
          <ul className="grid gap-x-8 gap-y-5 sm:grid-cols-2">
            {content.confirmationItems.map((item) => (
      ...
      <section className="section-spacing" aria-labelledby="chair-process-title">
        <div className="container-custom">
          <h2 id="chair-process-title" className="mb-12 font-sans text-3xl font-light text-[#241f1b] md:text-4xl">
            {content.processHeading}
          </h2>
          <ol className="grid gap-8 md:grid-cols-3">
            {content.processSteps.map((step, index) => {
              const Icon = index === 0 ? BriefcaseBusiness : index === 1 ? FileCheck2 : MessageCircle;
              return (
      ...
      <section className="section-spacing bg-[#241f1b] text-white" aria-labelledby="chair-inquiry-title">
        <div className="container-custom grid gap-10 lg:grid-cols-[44%_56%] lg:gap-16">
          <div>
            <h2 id="chair-inquiry-title" className="mb-5 font-sans text-3xl font-light md:text-4xl">
      ...
      <section className="section-spacing" aria-labelledby="chair-faq-title">
        <div className="container-custom grid gap-10 lg:grid-cols-[34%_66%] lg:gap-16">
          <h2 id="chair-faq-title" className="font-sans text-3xl font-light text-[#241f1b] md:text-4xl">
            {content.faqHeading}
          </h2>
      ...
      <section className="bg-[#eee5da] py-16 md:py-20" aria-labelledby="chair-closing-title">
        <div className="container-custom flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
          <div className="max-w-3xl">
            <h2 id="chair-closing-title" className="mb-4 font-sans text-3xl font-light text-[#241f1b] md:text-4xl">
